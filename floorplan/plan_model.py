    ...
    rooms = [
        ...
        Room("ot", "OPERATION THEATRE", "20'-0\" x 20'-0\"", (0.4, 6.0, 20.0, 20.0), "surgical",
             furniture=[("ottable", 8.6, 13.0, 2.6, 6.5, 15), ("counter", 1.0, 6.3, 6.0, 1.5, 0),
                        ...
                        ("cart", 4.0, 22.4, 1.8, 2.6, 0), ("counter", 13.0, 24.2, 5.5, 1.4, 0),
        ...
        Room("recovery", "RECOVERY / PRE-OP", "12'-0\" x 8'-0\"", (0.4, 26.4, 12.0, 8.0), "surgical",
             doors=[(6.0, 26.3, 3.0, "S"), (12.5, 30.4, 3.5, "W")],
             furniture=[("bed", 1.2, 27.2, 3.2, 6.6, 0), ("cart", 9.8, 27.0, 1.8, 2.2, 0)],
             notes="The ONLY patient way into the OT suite: corridor -> recovery -> OT"),
        Room("xray", "X-RAY / EMERGENCY", "12'-0\" x 7'-7\"", (0.4, 34.8, 12.0, 7.6), "consult",
        ...
        Room("opd2", "OPD 2", "10'-2\" x 7'-5\"", (20.9, 13.0, 10.2, 7.4), "consult",
             doors=[(24.5, 20.5, 2.6, "S")],
             furniture=[("desk", 22.0, 16.4, 4.6, 2.0, 0), ("chair", 23.4, 14.4, 1.4, 1.4, 0),
                        ("exam", 28.4, 13.3, 2.25, 5.2, 0)]),
        Room("opd1", "OPD 1", "10'-2\" x 7'-7\"", (20.9, 25.8, 10.2, 7.6), "consult",
             doors=[(24.5, 25.7, 2.6, "N")],
        ...
        Room("opd3", "OPD 3 (Sr. Consultant)", "≈12' x 12'-4\"", (31.6, 9.8, 12.6, 12.3), "consult",
             ...
             notes="Private door to the Doctors' Lounge; patient door from the OPD lobby"),
        ...
        Room("ns", "NURSING STN. + Obs Bed", "11'-10\" x 7'-7\"", (0.4, 61.4, 11.8, 7.6), "inpatient",
             ...
             furniture=[("counter", 0.8, 61.7, 8.0, 1.7, 0), ("chair", 2.6, 63.9, 1.4, 1.4, 0),
                        ("bed", 8.2, 63.9, 3.0, 4.6, 0), ("cart", 5.4, 66.6, 1.6, 2.0, 0)]),
        ...
        Room("room1", "ROOM 1", "≈23' x 10'-2\" (ensuite)", (18.2, 48.8, 25.0, 10.2), "inpatient",
             ...
             furniture=[("bed", 24.0, 49.6, 3.2, 6.6, 0), ("side", 27.6, 49.4, 1.4, 1.6, 0),
                        ("chair", 30.4, 50.0, 1.4, 1.4, 0), ("wardrobe", 38.4, 49.3, 2.0, 4.5, 0),
                        ("sofa", 33.0, 56.2, 5.5, 2.3, 0), ("tv", 30.0, 49.1, 3.6, 0.7, 0)]),
        Room("wcR1", "T", "", (18.4, 49.0, 4.2, 5.2), "wet",
             doors=[(20.4, 54.3, 2.0, "N")],
             furniture=[("wc", 19.0, 49.6, 1.5, 2.0, 0), ("sink", 21.4, 49.4, 1.2, 1.2, 0),
                        ("shower", 20.9, 52.4, 1.6, 1.6, 0)]),
        Room("room2", "ROOM 2", "≈22' x 10'-0\" (ensuite)", (18.2, 59.4, 24.0, 10.0), "inpatient",
             ...
             furniture=[("bed", 24.0, 60.2, 3.2, 6.6, 0), ("side", 27.6, 60.0, 1.4, 1.6, 0),
                        ("chair", 30.4, 60.6, 1.4, 1.4, 0), ("wardrobe", 37.4, 59.9, 2.0, 4.5, 0),
                        ("sofa", 32.4, 66.6, 5.5, 2.3, 0), ("tv", 30.0, 59.7, 3.6, 0.7, 0)]),
        Room("wcR2", "T", "", (18.4, 59.6, 4.2, 5.2), "wet",
             doors=[(20.4, 64.9, 2.0, "N")],
             furniture=[("wc", 19.0, 60.2, 1.5, 2.0, 0), ("sink", 21.4, 60.0, 1.2, 1.2, 0),
                        ("shower", 20.9, 63.0, 1.6, 1.6, 0)]),
        Room("room3", "ROOM 3", "≈21' x 9'-6\" (ensuite)", (18.2, 69.8, 22.5, 9.6), "inpatient",
             ...
             furniture=[("bed", 24.0, 70.6, 3.2, 6.6, 0), ("side", 27.6, 70.4, 1.4, 1.6, 0),
                        ("chair", 30.4, 71.0, 1.4, 1.4, 0), ("wardrobe", 36.2, 70.3, 2.0, 4.5, 0),
                        ("sofa", 31.4, 76.4, 5.5, 2.3, 0), ("tv", 30.0, 70.1, 3.6, 0.7, 0)]),
        Room("wcR3", "T", "", (18.4, 70.0, 4.2, 5.2), "wet",
             doors=[(20.4, 75.3, 2.0, "N")],
             furniture=[("wc", 19.0, 70.6, 1.5, 2.0, 0), ("sink", 21.4, 70.4, 1.2, 1.2, 0),
                        ("shower", 20.9, 73.4, 1.6, 1.6, 0)]),
        Room("room4", "ROOM 4", "≈17' x 11'-10\" (ensuite)", (18.0, 79.8, 18.0, 11.8), "inpatient",
             ...
             furniture=[("bed", 24.0, 80.8, 3.2, 6.6, 0), ("side", 27.6, 80.6, 1.4, 1.6, 0),
                        ("chair", 30.2, 81.2, 1.4, 1.4, 0), ("wardrobe", 32.4, 80.3, 2.0, 4.5, 0),
                        ("sofa", 27.0, 88.8, 5.5, 2.3, 0), ("tv", 24.0, 80.1, 3.6, 0.7, 0)]),
        Room("wcR4", "T", "", (18.2, 80.0, 4.2, 5.2), "wet",
             doors=[(20.2, 85.3, 2.0, "N")],
             furniture=[("wc", 18.8, 80.6, 1.5, 2.0, 0), ("sink", 21.2, 80.4, 1.2, 1.2, 0),
                        ("shower", 20.7, 83.4, 1.6, 1.6, 0)]),
        Room("suite", "SUITE ROOM 5", "12'-2\" x 8'-7\" (ensuite)", (0.4, 83.4, 12.2, 8.6), "inpatient",
             ...
             furniture=[("bed", 1.2, 84.6, 3.2, 6.6, 0), ("side", 4.8, 84.4, 1.4, 1.6, 0),
                        ("wardrobe", 10.4, 83.8, 1.9, 2.8, 0), ("sofa", 4.2, 89.4, 4.0, 2.2, 0),
                        ("tv", 0.9, 83.9, 3.2, 0.7, 0)]),
        Room("wcS5", "T", "", (8.6, 86.8, 3.8, 5.0), "wet",
             doors=[(8.5, 88.9, 2.0, "E")],
             furniture=[("wc", 9.2, 87.4, 1.5, 2.0, 0), ("sink", 11.2, 87.2, 1.2, 1.2, 0),
                        ("shower", 10.8, 90.1, 1.5, 1.5, 0)]),
        # --- circulation label anchors
        Room("link", "link", "", (20.9, 9.4, 10.2, 3.2), "circ"),
        Room("spine", "7'-8\" w. spine", "", (12.8, 26.2, 7.7, 16.6), "circ"),
        Room("opdlobby", "OPD LOBBY", "", (20.9, 20.8, 10.2, 4.6), "circ",
             notes="One entry feeds OPD 1, OPD 2 and (through) OPD 3"),
        Room("overflow", "overflow seating", "", (22.0, 34.0, 9.0, 5.5), "circ",
             furniture=[("chair", 22.6, 34.6, 1.4, 1.4, 0), ("chair", 24.4, 34.6, 1.4, 1.4, 0),
                        ("chair", 26.2, 34.6, 1.4, 1.4, 0), ("chair", 28.0, 34.6, 1.4, 1.4, 0),
                        ("chair", 22.6, 37.2, 1.4, 1.4, 0), ("chair", 24.4, 37.2, 1.4, 1.4, 0)]),
